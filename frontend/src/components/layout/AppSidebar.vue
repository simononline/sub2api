<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <router-link
        to="/home"
        class="sidebar-logo sidebar-home-logo-link flex h-9 w-9 items-center justify-center overflow-hidden rounded-lg border border-primary-400/30 bg-dark-950/5 dark:bg-dark-950"
        aria-label="Home"
        @click="closeMobile"
      >
      ...
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <router-link to="/home" class="sidebar-brand-title-link" aria-label="Home" @click="closeMobile">
          <span class="sidebar-brand-title text-lg font-normal text-gray-900 dark:text-white">
            {{ siteName }}
          </span>
        </router-link>
        <!-- Version Badge -->
    ...
    <nav class="sidebar-nav scrollbar-hide">
      ...
      <template v-if="isAdmin">
        ...
        <div v-if="publicNavItems.length" class="sidebar-section">
    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M12 15.75a4.5 4.5 0 100-9 4.5 4.5 0 000 9zM8.25 14.25L6.75 21 12 18l5.25 3-1.5-6.75M8.25 3.75L6.75 6m9-2.25L17.25 6'
        })
      ]
    )
}
...
const adminSelfNavSections = computed(() => splitSelfNavItems(finalizeNav(buildSelfNavItems(false, true))))

// Personal navigation items (for admin's "My Account" section, without Dashboard).
// Admins access 可用渠道 from this section; regular users keep it hidden from
// their sidebar.
...
.sidebar-home-logo-link:hover {
  border-color: rgb(0 197 115 / 0.5);
  opacity: 0.88;
}
...
.sidebar-home-logo-link:focus-visible {
  outline: 2px solid rgb(0 197 115 / 0.65);
  outline-offset: 2px;
}

.sidebar-brand-title-link:hover .sidebar-brand-title {
  color: rgb(0 168 98);
}

:global(.dark) .sidebar-brand-title-link:hover .sidebar-brand-title {
